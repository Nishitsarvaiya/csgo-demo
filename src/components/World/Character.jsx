...
export default function Character({ camera, ...restProps }) {
	const character = useRef();
	const { scene, nodes, materials, animations } = useGLTF('/models/Agent2/agent-2.gltf');
	...
	const prevAction = useRef(actions.Idle);
	...
	const characterState = (delta) => {
		...
		frameDecceleration.z = Math.sign(frameDecceleration.z) * Math.min(Math.abs(frameDecceleration.z), Math.abs(newVelocity.z));
	...
	useFrame((state, delta) => {
		...
		if (prevAction.current !== currAction.current) {
			prevAction.current?.fadeOut(0.6);
